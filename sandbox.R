...
# Выделяем тренировочный и контрольный датасеты для проверка качества на данных, не используемых при обучении
set.seed(24)
...
df_test_final$`Артериальная гипертензия` <- train_and_predict(formula_ag, df_data, df_test_final)
df_test_final$`ОНМК` <- train_and_predict(formula_onmk, df_data, df_test_final)
df_test_final$`Стенокардия, ИБС, инфаркт миокарда` <- train_and_predict(formula_st, df_data, df_test_final)
df_test_final$`Сердечная недостаточность` <- train_and_predict(formula_sn, df_data, df_test_final)
df_test_final$`Прочие заболевания сердца` <- train_and_predict(formula_another, df_data, df_test_final)
